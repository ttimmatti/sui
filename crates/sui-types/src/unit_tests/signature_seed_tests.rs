...
use crate::crypto::{BcsSignable, AuthorityKeyPair};
...
fn test_deterministic_addresses_by_id() {
    ...
    let sui_address_0_0 = seed.new_deterministic_address::<AuthorityKeyPair>(&id_0, Some(&TEST_DOMAIN));
    assert!(sui_address_0_0.is_ok());

    let sui_address_0_1 = seed.new_deterministic_address::<AuthorityKeyPair>(&id_0, Some(&TEST_DOMAIN));
    ...
    let sui_address_1_0 = seed.new_deterministic_address::<AuthorityKeyPair>(&id_1, Some(&TEST_DOMAIN));
...
fn test_deterministic_addresses_by_seed() {
    ...
    let sui_address_0 = seed_0.new_deterministic_address::<AuthorityKeyPair>(&TEST_ID, Some(&TEST_DOMAIN));
    assert!(sui_address_0.is_ok());

    let sui_address_1 = seed_1.new_deterministic_address::<AuthorityKeyPair>(&TEST_ID, Some(&TEST_DOMAIN));
...
fn test_deterministic_addresses_by_domain() {
    ...
    let sui_address_0 = seed.new_deterministic_address::<AuthorityKeyPair>(&TEST_ID, Some(&domain_0));
    assert!(sui_address_0.is_ok());

    let sui_address_1 = seed.new_deterministic_address::<AuthorityKeyPair>(&TEST_ID, Some(&domain_1));
    assert!(sui_address_1.is_ok());
...
fn test_deterministic_signing() {
    ...
    let sui_address_0 = seed
        .new_deterministic_address::<Ed25519KeyPair>(&id_0, Some(&TEST_DOMAIN))
        .unwrap();
    let sui_address_1 = seed
        .new_deterministic_address::<Ed25519KeyPair>(&id_1, Some(&TEST_DOMAIN))
        .unwrap();

    // Sign with both addresses.
    let sig_0 = seed.sign::<_, Ed25519KeyPair>(&id_0, Some(&TEST_DOMAIN), &msg0);
    assert!(sig_0.is_ok());
    let sig_0_ok = sig_0.unwrap();

    let sig_1 = seed.sign::<_, Ed25519KeyPair>(&id_1, Some(&TEST_DOMAIN), &msg0);
    assert!(sig_1.is_ok());
    ...
    let sig_0_1 = seed.sign::<_, Ed25519KeyPair>(&id_0, Some(&TEST_DOMAIN), &msg0).unwrap();
